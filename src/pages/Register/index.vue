<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>手机号:</label>
        <input placeholder="请输入你的手机号" v-model="phone" name="phone" v-validate="{ required: true, regex: /^1[3456789]\d{9}$/ }" :class="{ invalid: errors.has('phone') }" />
      ...
      <div class="content">
        <label>登录密码:</label>
        <input placeholder="请输入8-16个字符，至少1个大写小写字母和1个数字" v-model="password" name="password" v-validate="{ required: true, regex: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[^]{8,16}$/ }" :class="{ invalid: errors.has('password') }" />
        <span class="error-msg">{{ errors.first('password') }}</span>
      </div>
      <div class="content">
        <label>确认密码:</label>
        <input placeholder="确认您的密码" v-model="password1" name="password1" v-validate="{ required: true, is: password }" :class="{ invalid: errors.has('password1') }" />
