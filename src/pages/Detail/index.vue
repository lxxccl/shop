          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="saleAttr in spuSaleAttrList" :key="saleAttr.id">
                ...
                <!--每一个销售属性的属性值的地方-->
                <dd changepirce="0" :class="{ active: saleAttrValue.isChecked == 1 }" v-for="saleAttrValue in saleAttr.spuSaleAttrValueList" :key="saleAttrValue.id" @click="changeChecked(saleAttrValue, saleAttr.spuSaleAttrValueList)">{{ saleAttrValue.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
...
export default {
  ...
  methods: {
    changeChecked(saleAttrValue, arr) {
      // 先将所以的ischeckbox的值变为0，再把点击的那个变为1
      arr.forEach(item => {
        item.isChecked = '0'
      })
    ...
    //数量的表单元素的change回调
    handler(event) {
      //通过event事件对象获取用户输入内容[用户输入的内容一定是字符串类型的数据],非数字的乘法为NAN
