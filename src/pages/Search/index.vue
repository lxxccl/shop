    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="details clearfix">
            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{ active: isOne }" @click="sort(1)">
                  <a>综合 <span v-show="isOne" class="iconfont" :class="{ 'icon-UP': isDesc, 'icon-DOWN': isAsc }"></span></a>
                </li>
                <li :class="{ active: isTwo }" @click="sort(2)">
                  <a>价格 <span v-show="isTwo" class="iconfont" :class="{ 'icon-UP': isDesc, 'icon-DOWN': isAsc }"></span></a>
                </li>
              </ul>
            </div>
          </div>
          ...
          <!-- 分页器 -->
          <Pagination :total="total" :pageSize="searchParams.pageSize" :pageNo="searchParams.pageNo" :pagerCount="5" @getPageNo="getPageNo"></Pagination>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    // search模块需要发起多次请求，使用放在mounted只能发起一次，但定义成方法可以多次调用
    getData() {
